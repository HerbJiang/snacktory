...
public class MyOutputFormatter extends OutputFormatter {
    ...
    public MyOutputFormatter(int minParagraphText) {
        this(minParagraphText, NODES_TO_REPLACE);
    }

    public MyOutputFormatter(int minParagraphText, List<String> nodesToReplace) {
    ...
    public String getFormattedText(Element topNode) {
        ...
        append(topNode, sb, "p");
        String str = sb.toString();//SHelper.innerTrim(sb.toString());
        if (str.length() > 500)
            return str.trim();

        // no subelements
        if (str.isEmpty() || !topNode.text().isEmpty() && str.length() <= topNode.text().length())
            str = topNode.text().trim();
        
        // no subelements
        if (str.isEmpty() || !topNode.ownText().isEmpty() && str.length() <= topNode.ownText().length())
            str = topNode.ownText().trim();
